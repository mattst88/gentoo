...
IUSE="${IUSE_VIDEO_CARDS}
	...
	+llvm +nptl opencl osmesa pax_kernel openmax pic selinux test vaapi
...
multilib_src_configure() {
	...
	# FINISHME: no option for this in meson I can see
	# x86 hardened pax_kernel needs glx-rts, bug 240956
	# if [[ ${ABI} == x86 ]]; then
	# 	emesonargs+=( $(use_enable pax_kernel glx-read-only-text) )
	# fi
